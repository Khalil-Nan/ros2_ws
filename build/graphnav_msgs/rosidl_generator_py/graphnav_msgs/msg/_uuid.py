...
class UUID(metaclass=Metaclass_UUID):
    ...
    def __init__(self, **kwargs):
        ...
        if 'id' not in kwargs:
            self.id = numpy.zeros(16, dtype=numpy.uint8)
        else:
        ...
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                ...
                assert value.size == 16, \
                    "The 'id' numpy.ndarray() must have a size of 16"
                self._id = value
                return
            ...
            assert \
                ...
                "The 'id' field must be a set or sequence with length 16 and each value of type 'int' and each unsigned integer in [0, 255]"
